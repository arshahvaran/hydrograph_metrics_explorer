...
describe('repair 8: the DTW band follows the step of a resampled Plots-tab frame', () => {
  it('±12 hourly steps (the hourly default since events-05) become ±1 daily step on a daily resample', () => {
    const rows = ['date,observed,sim']
    ...
    expect(ds0.view.timingConfig.dtwBand).toBe(12)
    expect(ctxFor(ds0, frameFor(ds0)).timing.dtwBand).toBe(12)
    ...
    expect(ds.view.timingConfig.dtwBand).toBe(12)   // the setting itself is unchanged
  })
})

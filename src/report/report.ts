...
export function summaryPairs(ds: Dataset, frame: Frame): [string, string][] {
  ...
  return [
    ...
    ['Timing config', `events ≥ P${v.timingConfig.eventThreshold.value}${v.timingConfig.eventThreshold.kind === 'absolute' ? ' (abs)' : ''}, min event gap ${v.timingConfig.eventMinDistance}, peak window ±${v.timingConfig.peakMatchTolerance}, peak separation ${v.timingConfig.peakMinDistance}, DTW band ${Math.round(v.timingConfig.dtwBandFraction * 100)}%`],
  ];
}

...
export function bootstrapCIs(
  ...
  // the same pairs untransformed: the FDC signatures read them (design D2)
  ...
  const rro = new Float64Array(n), rrs = new Float64Array(n);
  ...
  for (let b = 0; b < B; b++) {
    ...
    for (let blk = 0; blk < nBlocks && k < n; blk++) {
      ...
      for (let j = 0; j < L && k < n; j++, k++) {
        ...
        ro[k] = paired.obs[idx];
        rs[k] = paired.sim[idx];
        rro[k] = pairedRaw.obs[idx];
        rrs[k] = pairedRaw.sim[idx];
      }
    }
    const { values } = classicalValues(ro, rs, { o: rro, s: rrs }, ctx.transform);
    // MASE keeps the naive scale of the original ordered record (spliced blocks inflate a replicate's own)
    if (Number.isFinite(values.mase)) values.mase = naive > 0 && Number.isFinite(values.mae) ? values.mae / naive : NaN;

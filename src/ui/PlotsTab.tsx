...
function PlotsTabInner({ ds }: { ds: Dataset }) {
  ...
  const { traces, layout, note } = useMemo(() => {
    ...
    if (plot === 'heatmap' || plot === 'spaghetti') {
      ...
      if (plot === 'heatmap') {
        return {
          traces: [{ z: years.map(y => byYear.get(y)!), x: Array.from({ length: 366 }, (_, i) => i + 1), y: years, type: 'heatmap', colorscale: 'Rainbow', colorbar: { title: { text: unit, side: 'right' }, lenmode: 'pixels', len: 370, y: 0.5, yanchor: 'middle', thickness: 14, outlinewidth: 0 } }],
